...
The following life-cycle methods are also called through the stack and you can override them inside your presenters and interactor classes. The activity onDestroy() contains an additional flag isChangingConfigurations(). 
...
1. The framework will always attempt to store the presenter and re-attach it when an activity is recreated after a screen rotation. There are certain instances where this is not guaranteed to happen, however:
...
Exercise caution if you store classes in your presenter/interactors that retain a reference to an activity context. If the presenter and interactor layers are retained when the activity is recreated, you'll end up with a memory leak. To avoid this...
- Use an application context for things like database operations, loading resource values and starting services. Avoid retaining an activity context reference for anything ui related, like inflating layouts, showing toasts or dialogs. 
- If you do require an activity context, use it on demand and pass it in as a method argument. Do not retain it as a class property so it can be garbage collected.  
